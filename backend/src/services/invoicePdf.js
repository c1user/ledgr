...
export function buildInvoicePdf(invoice, business, lang = "en") {
  ...
  return new Promise((resolve, reject) => {
    ...
    bizAddr.forEach((line, i) =>
      doc.text(line, left, 74 + i * 12, { width: 280 }),
    );
    ...
    doc.text(invoice.invoice_number || "", 330, 80, {
      width: right - 330,
      align: "right",
    });
    doc.fillColor(grayrgb).fontSize(9);
    doc.text(`${L.issueDate}: ${fmtDate(invoice.issue_date, lang)}`, 330, 96, {
      width: right - 330,
      align: "right",
    });
    doc.text(`${L.dueDate}: ${fmtDate(invoice.due_date, lang)}`, 330, 108, {
      width: right - 330,
      align: "right",
    });
    ...
    doc
      ...
      .font("Helvetica")
      .fontSize(8)
      .text(L.billTo.toUpperCase(), left, y);
    y += 14;
    ...
    const headRow = (yy) => {
      ...
      doc.text(L.description.toUpperCase(), cols.desc, yy, {
        width: colW.desc,
      });
      doc.text(L.qty.toUpperCase(), cols.qty, yy, {
        width: colW.qty,
        align: "right",
      });
      doc.text(L.unitPrice.toUpperCase(), cols.unit, yy, {
        width: colW.unit,
        align: "right",
      });
      doc.text(L.amount.toUpperCase(), cols.amount, yy, {
        width: colW.amount,
        align: "right",
      });
    };
    ...
    for (const item of invoice.line_items || []) {
      ...
      doc
        .fillColor("#191524")
        .text(item.description || "", cols.desc, y, { width: colW.desc });
      doc.fillColor("#403a52");
      doc.text(String(Number(item.quantity)), cols.qty, y, {
        width: colW.qty,
        align: "right",
      });
      doc.text(money(item.unit_price), cols.unit, y, {
        width: colW.unit,
        align: "right",
      });
      doc.text(money(item.total), cols.amount, y, {
        width: colW.amount,
        align: "right",
      });
    ...
    const totalLine = (label, value, opts = {}) => {
      ...
      doc
        .fillColor("#191524")
        .text(value, cols.amount, y, { width: colW.amount, align: "right" });
      y += opts.bold ? 22 : 16;
    };
    ...
    if (Number(invoice.tax_total) > 0) {
      ...
      totalLine(
        `${taxName} (${Number(invoice.tax_rate)}%)`,
        money(invoice.tax_total),
      );
    }
    doc
      .moveTo(cols.unit - 60, y)
      .lineTo(right, y)
      .lineWidth(1.5)
      .strokeColor("#191524")
      .stroke();
    ...
    if (invoice.notes) {
      ...
      doc
        ...
        .font("Helvetica")
        .fontSize(8)
        .text(L.notes.toUpperCase(), left, y);
      y += 12;
      doc
        .fillColor("#4f4a60")
        .fontSize(10)
